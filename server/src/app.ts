...
import { heartbeatAllocationRoutes } from "./routes/heartbeat-allocations.js";
import { routingGovernanceRoutes } from "./routes/routing-governance.js";
...
export async function createApp(
  ...
  api.use(refineryRoutes(db));
  api.use(approvalRoutes(db, { pluginWorkerManager: workerManager }));
  api.use(secretRoutes(db));
  api.use(costRoutes(db, { pluginWorkerManager: workerManager }));
  api.use(heartbeatAllocationRoutes(db));
  api.use(routingGovernanceRoutes(db));
  api.use(auditEventRoutes(db));
